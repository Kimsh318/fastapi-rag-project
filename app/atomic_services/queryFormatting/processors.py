...
class BaseQueryFormattingProcessor:
    """Base Query Formatting Processor"""
    ...
    async def _get_documents_by_ids(self, list_doc_ids: list):
        """주어진 문서 ID 리스트에 대한 문서 텍스트를 검색하여 반환합니다."""
        es_results = await self.es_client.mget(body={
            "ids": list_doc_ids
    ...
    def _merge_chunks(self, list_docs, MAX_PROMPT_LEN, tokenizer):
        """문서 청크들을 순회하며 각 청크를 처리하고, 유효한 청크들을 병합합니다."""
        merged_chunk_texts = ""
        sum_of_token_ids = 0
        num_of_doc = 1
        list_doc_ids = []
        ...
        for output in list_docs:
            ...
            # 합친 텍스트의 토큰 수가 MAX_PROMPT_LEN을 초과하면 반복 종료
...
class DevelopmentQueryFormattingProcessor(BaseQueryFormattingProcessor):
    """Development 환경용 Query Formatting Processor"""
...
class ProductionQueryFormattingProcessor(BaseQueryFormattingProcessor):
    """Production 환경용 Query Formatting Processor"""
