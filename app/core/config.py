...
class BaseConfig:
    ...
    APP_ENVIRONMENT: str = (
        "development"  # 앱사용환경 지정 : prototype, development, production
    )
    APP_SERVER: str = (
        "ML1"  #  서버이름 지정 : ML1, serving1, serving2 (production 환경이라면 필수)
    )
...
class PrototypeConfig(BaseConfig):
    ...
    # 공통 설정
    # ---------------------------------
    # 정적파일 경로 : docs(swagger-ui) 렌더링에 필요한 js, css 등
    STATIC_PATH = "/app/app/static/"  # 정적파일 경로 : docs(swagger-ui) 렌더링에 필요한 js, css 등
    SERVER_IP: str = "None"
    ...
    VECTOR_DOC_COLLECTION_NM: str = (
        "banking_law_collection"  # 임베딩 검색을 위한 문서 컬렉션 이름
    )
    ...
    REFINE_FAISS_FAISS_INDEX_FILE_PATH: str = (
        "/app/app/db_faiss(will_be_removed)/vector_index.faiss"
    )
    ...
    FAISS_SEARCH_K: int = (
        30  # semantic search 결과의 top_k와는 별개의 값. search_k > top_k
    )
    ...
    RERANKER_EXCLUDED_DOCS: List[str] = [
        "상공인대출지침",
        "선박금융 온렌딩대출 취급지침",
    ]  # Rerank 제외 문서 목록
...
class DevelopmentConfig(BaseConfig):
    ...
    # 정적파일 경로 : docs(swagger-ui) 렌더링에 필요한 js, css 등
    STATIC_PATH = (
        "./app/static/"  # 정적파일 경로 : docs(swagger-ui) 렌더링에 필요한 js, css 등
    )
    ...
    REFINE_FAISS_FAISS_INDEX_FILE_PATH: str = (
        "/workspace/dev/dev1_p/13_langchain_fastapi_backend_dev/v18/app/db_faiss/faiss_useless_terms.index"
    )
    REFINE_FAISS_METADATA_FILE_PATH: str = (
        "/workspace/dev/dev1_p/13_langchain_fastapi_backend_dev/v18/app/db_faiss/faiss_useless_terms_meta.json"
    )
    ...
    FAISS_INDEX_FILE_PATH: str = (
        "/workspace/dev/dev1_p/13_langchain_fastapi_backend_dev/v18/app/db_faiss/faiss_documents.index"
    )
    FAISS_METADATA_FILE_PATH: str = (
        "/workspace/dev/dev1_p/13_langchain_fastapi_backend_dev/v18/app/db_faiss/faiss_documents_meta.json"
    )
    FAISS_MODEL_DEVICE_ID: str = "cuda:3"
    FAISS_INDEX_DEVICE_ID: str = "cuda:3"
    FAISS_SEARCH_K: int = (
        100  # semantic search 결과의 top_k와는 별개의 값. search_k > top_k
    )
    ...
    ELASTICSEARCH_DOC_DB_NM: str = (
        ES_INDEX_NAME  #'esdb_0827'       # 모든 문서가 저장된 Elasticsearch Index 이름
    )
    ELASTICSEARCH_LOG_DB_NM: str = (
        "logging"  # 사용자의 사용 이력이 저장되는 Elasticsearch Index 이름
    )
    NON_CRITICAL_DOCS: List[str] = [
        "상공인대출지침",
        "선박금융 온렌딩대출 취급지침",
    ...
    NON_CRITICAL_DOCS_KEYWORDS: List[str] = [
        "상공인",
        "일반고객",
        "선박금융",
        "온렌딩",
...
class ProductionConfig(BaseConfig):
    ...
    if BaseConfig.APP_SERVER == "serving1":
        REFINE_FAISS_FAISS_INDEX_FILE_PATH: str = (
            "/workspace/1_product/faiss/faiss_useless_terms.index"
        )
        REFINE_FAISS_METADATA_FILE_PATH: str = (
            "/workspace/1_product/faiss/faiss_useless_terms_meta.json"
        )
    elif BaseConfig.APP_SERVER == "serving2":
        REFINE_FAISS_FAISS_INDEX_FILE_PATH: str = (
            "/workspace/2_product/faiss/faiss_useless_terms.index"
        )
        REFINE_FAISS_METADATA_FILE_PATH: str = (
            "/workspace/2_product/faiss/faiss_useless_terms_meta.json"
        )
    ...
    if BaseConfig.APP_SERVER == "serving1":
        ...
        FAISS_METADATA_FILE_PATH: str = (
            "/workspace/1_product/faiss/faiss_documents_meta.json"
        )
    elif BaseConfig.APP_SERVER == "serving2":
        FAISS_INDEX_FILE_PATH: str = "/workspace/2_product/faiss/faiss_documents.index"
        FAISS_METADATA_FILE_PATH: str = (
            "/workspace/2_product/faiss/faiss_documents_meta.json"
        )
    ...
    if BaseConfig.APP_SERVER == "serving1":
        ES_API_HOST: str = "http://127.0.0.1:9200"
        ES_API_SUB_HOST: str = (
            "http://10.6.40.79:32110"  # Serving 1번 ES 장애시, Serving 2번의 ES를 사용하도록 하기 위함
        )
    elif BaseConfig.APP_SERVER == "serving2":
        ES_API_HOST: str = "http://127.0.0.1:9200"
        ES_API_SUB_HOST: str = (
            "http://10.6.40.78:32110"  # Serving 2번 ES 장애시, Serving 1번의 ES를 사용하도록 하기 위함
        )
...
def get_settings():
    ...
    raise ValueError(
        f"Invalid APP_ENVIRONMENT: {app_env}. Please set it to 'prototype', 'development', or 'production'."
    )
